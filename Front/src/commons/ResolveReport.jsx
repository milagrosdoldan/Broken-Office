...
const ResolveReport = () => {
  ...
  const rejectedReport = (e) => {
    axios.put(`http://localhost:3001/api/report/rejectedreport/${params.id}`,{} , { withCredentials: true }).then(() => {
      Swal.fire({
        icon: "error",
  ...
  const resolveReport = async () => {
    try {
      await axios.put(`http://localhost:3001/api/report/solvereport/${params.id}`,{}, { withCredentials: true });
      Swal.fire({
        icon: "success",
